  ...
  if order
    pdf.indent 4 do
      ...
      pdf.font "Helvetica", :style => :bold do
        pdf.text "#{order.member.last_name}, #{order.member.first_name} - #{order.member.phone_number} - #{@pickup.name}", :size => 12
      end
  ...
  pdf.grid.rows.times do |row|
    pdf.grid.columns.times do |col|
      ...
      pdf.bounding_box box.top_left, :width => box.width, :height => box.height do
        render_order pdf, @pickup.orders[order_count]
        pdf.stroke { pdf.rectangle(pdf.bounds.top_left, box.width,box.height)}
        order_count += 1
      end
    end
  end
  if order_count % 10 == 0 && @pickup.orders[order_count] != nil
    pdf.start_new_page
